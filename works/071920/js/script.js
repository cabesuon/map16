require([
  ...
  */
    ...
    popupTemplate: {
      content: [
        ...
        },
        {
          type: 'text',
          text:
            'Predominant level in this cluster is <b>{cluster_type_ALERT_LEVEL}</b>.'
        }
      ]
    }
  };
  ...
  function sensorsPopupTemplateContent(feature) {
    ...
    div.innerHTML =
      ...
      + popupTemplateContentAttrTable([
        ...
        {
          value: safeAttrValue(attrs.PERCENT_WATER_LEVEL),
          label: '% Water Level'
        },
        {
          value: safeAttrValue(attrs.WATER_LEVEL),
          label: 'Water Level'
        },
        {
          value: safeAttrValue(attrs.DEVICE_ID),
          label: 'Device ID'
        },
        {
          value: safeAttrValue(attrs.MANHOLE_REF),
          label: 'MH Reference'
        },
        {
  ...
  const sensorsPopupTemplate = {
    title: 'Flood Sensor: {ALERT_LEVEL}',
    lastEditInfoEnabled: false,
    content: sensorsPopupTemplateContent,
    outFields: [
      'ALERT_LEVEL', 'DAM_AREA', 'POSTCODE', 'ROAD', 'MANHOLE_REF', 'DEVICE_ID', 'ALERT_LEVEL', 'ANALYTICS_URL', 'TOWN', 'PERCENT_WATER_LEVEL', 'WATER_LEVEL'
    ]
  };

  const sensorsLayer = new FeatureLayer({
    url: `${servicesUrl}/sus_uu_live_flood_sensors_wgs_master/FeatureServer/0`,
    outFields: [
      'ALERT_LEVEL', 'DAM_AREA', 'POSTCODE', 'ROAD', 'MANHOLE_REF', 'DEVICE_ID', 'ALERT_LEVEL', 'ANALYTICS_URL', 'TOWN', 'PERCENT_WATER_LEVEL', 'WATER_LEVEL'
    ],
  ...
  */
  ...
  const raingridPopupTemplate = {
    ...
    outFields: [
      'ALERT_LEVEL', 'MEASURE', 'POSTCODE', 'DAM_AREA', 'TOWN', 'DATE_ID', 'ANALYTICS_URL'
    ]
  };
  ...
  */
  ...
  const camerasLayer = new FeatureLayer({
    url: `${servicesUrl}/sus_uu_live_cam_monit_wgs_master_view/FeatureServer/0`,
    outFields: [
      'CAMERA_ID', 'CAMERA_EMBED', 'DAM_AREA', 'POSTCODE', 'THOROUGHFARE', 'TOWN', 'CAMERA_URL', 'CAMERA_ANALYTICS'
    ],
  ...
  */
  ...
  const riverSensorsPopupTemplate = {
    title: 'River Sensor: {ALERT_LEVEL}',
    lastEditInfoEnabled: false,
  ...
  const riverSensorsLayer = new FeatureLayer({
    url: `${servicesUrl}/uu_riv_ntwrk_wgs_master/FeatureServer/0`,
  ...
  function sensorsElementClickHandler(event) {
    ...
    sensorsLayerView.filter = {
      where: `ALERT_LEVEL='${selectedSensorLevel}'`
    };
    listNodeReset();
    if (selectedSensorLevel) {
