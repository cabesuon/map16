require([
 ...
 const sensorlayer = new FeatureLayer({
 ...
 const gatewaylayer = new FeatureLayer({
    url: `${servicesUrl}/sus_uu_gateway_install_view/FeatureServer/0`,
    outFields: [
      'gateway_id', 'asset_location','asset_location_other','gateway_installed'
    ],
    // featureReduction: clusterConfig, // polygon layer can not be reducted
    popupTemplate: popupTemplate
  });
  ...
  const map = new Map({
    basemap: 'gray-vector',
    layers: [DAMlayer, IDASlayer, raingridlayer,cameralayer,sensorlayer,gatewaylayer]
  });
